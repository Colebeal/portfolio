...
export default function Menu ({menuStatus, setSlideIndex, currentFolder, setCurrentFolder, menuHandler, returnHomeHandler}) {
  ...
  const menuStyles = {
    showMain: 'z-40 opacity-100 fixed flex flex-col top-[12%] right-5 md:top-0 md:right-0 h-[75%] md:h-full w-[90%] md:w-[30%] bg-orange-50 dark:bg-teal-950 animate-fade-in',
    hideMain: 'z-40 hidden fixed top-0 right-0 h-full md:w-[30%] bg-orange-50 dark:bg-teal-950 ease-in-out',
    showBkg: 'z-30 opacity-60 fixed top-0 left-0 h-full w-full bg-black animate-fade-in-Bkg',
    hideBkg: 'z-30 hidden fixed top-0 left-0 h-full w-full bg-black ease-in-out',
    //menuNav
