...
export default function MiniMenu({ isGallery, menuHandler }) {
  const mobileToggles = {
    slider:
      "basis-1/5 z-40 flex md:flex-col  w-fit h-fit p-2 rounded-md gap-4 bg-orange-100 dark:bg-teal-900 fixed top-10 right-5 shadow-lg",
    gallery:
  ...
  return (
    <div className={isGallery ? mobileToggles.gallery : mobileToggles.slider}>
      <ThemeToggle />
      <Link to="..">
        <HiOutlineHome className=" cursor-pointer text-2xl text-teal-900 dark:text-orange-50" />
      </Link>
      <HiMenu
        onClick={menuHandler}
